...
#define mon_days(yy, mm) (pmonthDays[Date::isleap(yy)][mm])

constexpr static const char delimeter = ' ';
constexpr static int pmonthDays[][13] = {
    {0, 31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31},
    {0, 31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31}};

constexpr bool Date::is_valid(int day, int month, int year) {
  return year >= Date::year_base && month > 0 && month <= 12 && day > 0 &&
         day <= pmonthDays[Date::isleap(year)][month];
}

Date::Date()
    : Date(1, 1, 1900) // Delegating ctor
{}
...
int Date::get_month_day() const { return md; }

int Date::get_month() const { return mm; }

int Date::get_year() const { return my; }
...
Date::WeekDay Date::get_week_day() const {
  ...
  static constexpr int doomsdayArrayLeapYear[] = {0, 4, 1, 7, 4, 2, 6,
                                                  4, 1, 5, 3, 7, 5};
  static constexpr int doomsdayArrayNotLeapYear[] = {0, 3, 7, 7, 4, 2, 6,
                                                     4, 1, 5, 3, 7, 5};
...
Date Date::operator-(int day) const { return Date{*this} -= day; }
...
Date &Date::operator--() { return (*this).operator-=(1); }
...
std::ostream &operator<<(std::ostream &os, const Date &date) {
  using namespace std;
  static const char *const pdays[] = {"Sunday",    "Monday",   "Tuesday",
                                      "Wednesday", "Thursday", "Friday",
                                      "Saturday"};

  os << setfill('0') << setw(2) << date.md;
  os << setfill(' ') << left;
  os << " " << date.mm << " " << date.my << " "
...
Date Date::random_date() {
  ...
  std::uniform_int_distribution<int> distribution1(random_min_year,
                                                   random_max_year);
  ...
  std::uniform_int_distribution<int> distribution3(
      0, (pmonthDays[isleap(year)][month]));
